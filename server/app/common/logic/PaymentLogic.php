...
class PaymentLogic extends BaseLogic
{
    ...
    public static function getPayWay($userId, $terminal, $params)
    {
        try {
            ...
            if ($params["from"] == "party") {
                // 党费缴纳
                $order = PartyFeeOrder::findOrEmpty($params['order_id'])->toArray();
            }
        ...
        try {
            switch ($params['from']) {
                case 'recharge':
                    ...
                    break;
                case 'party':
                    $order = PartyFeeOrder::findOrEmpty($params['order_id']);
                    if ($order->isEmpty()) {
                        throw new \Exception('充值订单不存在');
                    }
                    break;
    ...
    public static function pay($payWay, $from, $order, $terminal, $redirectUrl)
    {
        ...
        switch ($from) {
            case 'recharge':
                ...
                break;
            case 'party':
                PartyFeeOrder::update(['pay_way' => $payWay, 'pay_sn' => $paySn], ['id' => $order['id']]);
                break;
        }
