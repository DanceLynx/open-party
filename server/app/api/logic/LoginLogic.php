...
class LoginLogic extends BaseLogic
{
     ...
     * @date 2023-06-24
     */
    protected static function addIntegral(int $userId): void
    {
        $integral = 3;
        // 查询当天是否被重复记录过
        $today = date('Y-m-d');
        $todayLogin = IntegralLog::where('user_id', $userId)
            ->where('type', IntegralLog::TYPE_LOGIN)
            ->order(["id DESC"])
            ->findOrEmpty();

        if ($todayLogin->isEmpty() || $today != date('Y-m-d', strtotime($todayLogin->create_time))) {
            IntegralLogLogic::add($userId, IntegralLog::TYPE_LOGIN, $integral);
        }
    }
