...
use app\common\model\fee\PartyFeeTime;
...
class UserLogic extends BaseLogic
{
    ...
    public static function info(int $userId)
    {
        $user = User::where(['id' => $userId])
            ->field('id,sn,sex,account,password,nickname,real_name,department,member_level,avatar,mobile,create_time,user_money')
            ->append(["department_text", "member_level_text"])
            ->findOrEmpty();
        $user['has_password'] = !empty($user['password']);
     ...
     * @date 2023-06-25
     */
    public static function getIntegral(int $userId): int
    {
        $integral = User::where(['id' => $userId])->value('user_integral');
        return $integral ? (int)$integral : 0;
    }
     ...
     * @date 2023-06-25
     */
    public static function setIntegral(int $userId, int $integral): void
    {
        User::update(['user_integral' => $integral], ['id' => $userId]);
    }
    /**
     * @notes 设置用户信息
     * @param int $userId
     ...
     * @date 2023-06-20
     */
    public static function getPartyTime(int $userId): array
    {
        //查询用户的党费设置情况 如果是0 直接返回0
        $user = User::find($userId);

        if (floatval($user->party_fee) <= 0.00) {
            return [];
        }
        // 获取所有的应缴月份
        $partyTimes = PartyFeeTime::order("month", "desc")->select();
        // 读取用户所有的缴纳订单
        $partyOrders = PartyFeeOrder::where('user_id', $userId)->select();
        if ($partyTimes->isEmpty()) {
            return [];
        }
        $list = ["all" => [], "payed" => [], "paying" => []];
        foreach ($partyTimes as $partyTime) {
            if ($partyOrders->filter(function ($item) use ($partyTime) {
                return $item->month == $partyTime->month && $item->status == 1;
            })->isEmpty()) {
                $temp = [
                    "month" => $partyTime->month,
                    "fee" => $user->party_fee,
                    "status" => "0",
                    "pay_time" => "",
                ];
                ...
                $temp =  [
                    ...
                    "fee" => $user->party_fee,
                    "status" => "1",
                    "pay_time" => $partyOrders->filter(function ($item) use ($partyTime) {
                        return $item->month == $partyTime->month;
                    })->first()->pay_time,
                ];
                $list["payed"][] = $temp;
            }
