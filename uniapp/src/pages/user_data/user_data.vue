<template>
	...
	<!-- 党支部 -->
	<view class="item text-nr flex justify-between" @click="changeDepartment">
		<view class="label">党支部</view>
		<view class="content">{{ userInfo?.department_text || '未设置' }}</view>
		<u-icon name="arrow-right" size="22" color="#666"></u-icon>
	</view>
	<!-- 党内职务 -->
	<view class="item text-nr flex justify-between" @click="changeMemberLevel">
		<view class="label">党内职务</view>
		<view class="content">{{ userInfo?.member_level_text || '未设置'}}</view>
		<u-icon name="arrow-right" size="22" color="#666"></u-icon>
	</view>
	<!-- 手机号 -->
	<view class="item text-nr flex justify-between">
		...
		<!-- #ifdef MP-WEIXIN -->
		<u-button open-type="getPhoneNumber" @getphonenumber="getPhoneNumber" type="primary" shape="circle" size="mini"
			:plain="true">
			...
			<form @submit="changeNameConfirm">
				...
				<view class="mt-[80rpx]">
					<button class="bg-primary text-white w-full h-[80rpx] !text-lg !leading-[80rpx] rounded-full"
						form-type="submit" size="mini" hover-class="none">
						确定
					</button>
				</view>
			</form>
	...
	<!-- 性别修改组件 -->
	<u-picker mode="selector" v-model="showPicker" confirm-color="#FB342A" :default-selector="[0]" :range="sexList"
		@confirm="changeSexConfirm" />
	<!-- 党支部修改组件 -->
	<u-picker mode="selector" v-model="showDepartmentPicker" confirm-color="#FB342A" :default-selector="[0]"
		:range="userInfo?.departments" range-key="name" @confirm="changeDepartmentConfirm" />
	<!-- 党内职务修改组件 -->
	<u-picker mode="selector" v-model="showMemberLevelPicker" confirm-color="#FB342A" :default-selector="[0]"
		:range="userInfo?.member_levels" range-key="name" @confirm="changeMemberLevelConfirm" />
...
<script lang="ts" setup>
	...
	//显示党支部选择弹窗
	const showDepartmentPicker = ref<boolean | null>(false)
	//显示党内职务选择弹窗
	const showMemberLevelPicker = ref<boolean | null>(false)
	...
	const getUser = async () : Promise<void> => {
		...
		console.log(userInfo.value.departments)
	}
	...
	const changeDepartment = () => {
		...
		fieldType.value = FieldType.DEPARTMENT
	}
	// 显示修改党内职务弹窗
	const changeMemberLevel = () => {
		showMemberLevelPicker.value = true
		fieldType.value = FieldType.MEMBER_LEVEL
	}
	...
	// 修改党支部
	const changeDepartmentConfirm = (value) => {
		setUserInfoFun(userInfo.value?.departments[value[0]].value)
		showDepartmentPicker.value = false
	}
	// 修改党内职务
	const changeMemberLevelConfirm = (value) => {
		setUserInfoFun(userInfo.value?.member_levels[value[0]].value)
		showMemberLevelPicker.value = false
	}
