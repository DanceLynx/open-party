<template>
	<view class="u-card" @tap.stop="click"
		:class="{ 'u-border': border, 'u-card-full': full, 'u-card--border': borderRadius > 0 }" :style="{
		...
		<view v-if="showHead" class="u-card__head" :style="[{padding: padding + 'rpx'}, headStyle]" :class="{
			...
			<slot name="head" v-if="$slots['head']" />
		</view>
		<view @tap="bodyClick" class="u-card__body" :style="[{padding: padding + 'rpx'}, bodyStyle]">
			<slot name="body" />
		</view>
		<view v-if="showFoot" class="u-card__foot" @tap="footClick"
			:style="[{padding: $slots.foot ? padding + 'rpx' : 0}, footStyle]" :class="{
